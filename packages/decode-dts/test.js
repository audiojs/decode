...
import decode, { decoder } from './decode-dts.js'
...
t('stereo DTS: rate, duration, > 15 dB SNR per channel', async () => {
	let r = await decode(fx('stereo.dts'))
...
t('5.1 DTS: six channels in WAV order FL FR FC LFE BL BR', async () => {
	let r = await decode(fx('surround51.dts'))
	is(r.channelData.length, 6)
	// ground truth: `ffmpeg -i surround51.dts -f f32le -ac 6` yields these tones in FL FR FC LFE BL BR order
	is(r.channelData.map(d => tone(d, r.sampleRate)), [880, 220, 440, 60, 330, 1760])
})

t('streaming: 777-byte chunks equal whole-file', async () => {
	let bytes = fx('stereo.dts'), whole = await decode(bytes)
